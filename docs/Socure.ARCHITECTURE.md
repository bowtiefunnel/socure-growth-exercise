...
| 7 | **Enable — battle cards** | Hot/warm accounts only; 7-section card; approved proof points only; Langfuse groundedness eval (uncited claim = failed card) | separate repo: [`socure-battle-cards`](https://github.com/bowtiefunnel/socure-battle-cards) — `tools/battle-card.ts` + `prompts/battle-card.md` | NEURAL → **HUMAN gate** | grounding corpus + blocklist/allowlist | `battle_cards/<domain>.md`; rep reviews before use (never-graduate: outward-facing) |
