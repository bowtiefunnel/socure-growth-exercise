...
| 5 | **TargetCo angle + product** ② | • {{need}} → **{{product — KYC/CIP + Sigma / Sigma Identity/Synthetic/Device / RiskOS / Signals}}**<br>• One bullet per need→product mapping; only map needs with evidence in §1/§3/§7 |
...
3. **Two evidence classes, never mixed:** account facts carry source + confidence
   (enrichment or research, cap 0.8); TargetCo claims come only from `docs/grounding/`.
4. **Competitive statements always attributed** ("TargetCo positions…").
